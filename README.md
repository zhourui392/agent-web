...
- **管理台**（`/admin`，数据库 ADMIN 角色鉴权）— 使用概览、对话浏览、工作流管理、用户建议 triage、RAG 语料维护、运行时设置（Agent 模型）
...
> 流式聊天使用 `POST /api/chat/session/{id}/message/stream`，JSON body 包含 `message`、`resumeId`、`env` 和 `recall`（默认 `true`）。正文不再放进 URL 查询串，避免被代理日志、浏览器历史和请求行限制截获。
...
│                 workflow/、git/、suggestion/、metrics/、refinery/、issuelog/、setting/、*Properties
└── config/       Web MVC 配置
