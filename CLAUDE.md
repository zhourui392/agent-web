...
7. **召回拦截**: 前端"RAG 召回"开关(默认开, 持久化 localStorage)随 `POST /api/chat/session/{id}/runs` 的 JSON body 提交，经 `ChatRunController` → `ChatRunAppServiceImpl` 持久化后由 `ChatRunExecutor` 调 `RefineryRecaller.traceForChat`; 命中时把增强输入交给 `ChatRunPromptBuilder`，并通过可回放 `recall` 事件展示命中摘要。斜杠展开、历史前缀和最终回答约束统一在 `ChatRunPromptBuilder` 处理
