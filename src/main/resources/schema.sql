...
CREATE TABLE IF NOT EXISTS workbench (
    ...
    CHECK (worktree_path IS NULL OR length(worktree_path) BETWEEN 1 AND 4096),
    CHECK (worktree_branch IS NULL OR length(worktree_branch) BETWEEN 1 AND 512),
    CHECK (use_worktree = 0 OR worktree_path IS NOT NULL),
