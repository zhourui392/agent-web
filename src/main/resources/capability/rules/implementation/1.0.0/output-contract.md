...
`command`、`phase`（`RED` / `GREEN` / `VERIFY`）和 `exitCode`。Workbench 会把这些声明
...
`stage` 必须为 `IMPLEMENTATION`。`artifacts` 必须是 JSON 数组（`[...]`），恰好包含 `CHANGED_FILES`、`TEST_EVIDENCE`、
`IMPLEMENTATION_SUMMARY`、`TRACEABILITY` 各一个元素；不得用对象按 `artifactType` 做 key。每个数组元素只能包含 `artifactId`、
