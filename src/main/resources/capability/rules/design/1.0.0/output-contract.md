...
`stage` 必须为 `DESIGN`。`artifacts` 必须是 JSON 数组（`[...]`），恰好包含 `SOLUTION`、`CHANGE_PLAN`、`TEST_STRATEGY`、
`DEPLOYMENT_PLAN`、`ROLLBACK_PLAN`、`TRACEABILITY` 各一个元素；不得用对象按 `artifactType` 做 key。每个数组元素只能包含 `artifactId`、
