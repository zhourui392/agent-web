package com.example.agentweb.infra.database;
...
public class SqliteInitializer {
    ...
    private static final String CHAT_SESSION_TABLE = "chat_session";
    ...
    public void init() throws Exception {
        ...
        requireStageOnlyChatSessionSchema();
    ...
    private void requireStageOnlyChatSessionSchema() {
        String tableDefinition = jdbc.queryForObject(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name=?",
                String.class, CHAT_SESSION_TABLE);
        String normalized = tableDefinition == null
                ? ""
                : tableDefinition.toUpperCase(Locale.ROOT);
        if (!normalized.contains("'WORKBENCH_STAGE'")
                || normalized.contains("'WORKBENCH_PHASE'")) {
            throw new IllegalStateException(
                    "Stage-only chat_session schema is required; "
                            + "automatic Workbench migration is disabled");
        }
    }
