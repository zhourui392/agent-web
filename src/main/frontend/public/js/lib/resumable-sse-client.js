 ...
 */
export function parseFrames(buffer, consumer) {
...
export function shouldApply(lastSequence, event) {
  var sequence = numericSequence(event);
  return sequence == null || sequence > Number(lastSequence || 0);
}

export function nextDelay(attempt, baseMs, maxMs, jitter) {
...
export function open(url, options) {
  ...
  var fetchFn = options.fetch || window.fetch;
...
// 浏览器: 挂全局 window.AgentResumableSse (兼容未改的消费者)
if (typeof window !== 'undefined') {
  window.AgentResumableSse = {
