...
test('停止: 发送中点停止 → 中断并提示已手动停止;SSE URL 默认带 recall=true', async ({ page }) => {
  await gotoReady(page);

  // 截停 SSE 请求:挂住不响应,EventSource 停在 CONNECTING,sending 维持 true,"停止"按钮可点
  ...
  expect(req.url(), 'RAG 默认开 → SSE URL 带 recall=true').toContain('recall=true');
