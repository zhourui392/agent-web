...
- `/api/workbenches` 负责 Owner 范围内的 Workbench、阶段、会话、Run、Handoff、Capability、文档、附件和 Operation。
- `/api/admin/workbenches` 只向 `ADMIN` 暴露安全投影以及 Stop/Reconcile 运维动作。

完整方法、请求体、幂等键和并发版本约束以 [`interfaces/workbench`](../src/main/java/com/example/agentweb/interfaces/workbench) 下的 Controller 为准。Workbench 的 Repository Scope、人工授权和 Admin 投影边界见[技术设计总览](workbench/README.md)。
