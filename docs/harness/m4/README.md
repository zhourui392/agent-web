...
预期 Prompt/Skill 证据。正式 Harness Runtime 仍使用隔离 `HOME/CODEX_HOME`，不会读取该登录态；启用前
仍需由部署系统提供显式 Provider Credential Reference。当前剩余强制退出项为：

- 一个真实需求完成四阶段、经批准 local 部署、业务 AC 和最终报告。

Stub、Fake MCP、在线 Prompt/Skill Smoke 和 E2E 部署模板都不替代真实需求验收。在提供正式 Runtime
受控凭据并完成该步骤前，Harness 继续默认关闭，也不开放 test/production 部署。
...
5. 在线 Provider 验收默认复用当前用户的 Codex CLI 登录态；需要复核生产隔离认证路径时，显式切换为 `isolated-key` 并提供受控凭据引用。
...
1. 执行默认 `local-login` 的 `scripts/harness-m4/verify-online-provider.sh`，验证在线 Provider 收到真实 Prompt 和四个 Stage Skill；CLI/MCP/取消离线兼容合同已经通过，无需重复冒充在线验收。
2. 正式启用 Harness Runtime 前，由部署系统提供受控 Provider 凭据并配置 `AGENT_HARNESS_CODEX_CREDENTIAL_REFERENCE`；Runtime 仍不得读取或改写用户级认证目录，可用脚本的 `isolated-key` 模式复核该路径。
