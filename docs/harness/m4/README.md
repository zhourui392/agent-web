...
在线验收脚本已使用当前用户的系统默认 Codex 登录态完成四阶段真实 Provider 调用，四个 Stage 均返回
预期 Prompt/Skill 证据。正式 Harness Runtime 默认使用 `local-login`，由系统 `codex` 按服务账户的
`HOME/CODEX_HOME` 使用现有登录态；Adapter 不直接读取、复制或改写认证文件。需要生产多用户隔离时可
显式切换为 `isolated-key`，此时才要求 Provider Credential Reference。当前剩余强制退出项为：

- 一个真实需求完成四阶段、经批准 local 部署、业务 AC 和最终报告。

Stub、Fake MCP、在线 Prompt/Skill Smoke 和 E2E 部署模板都不替代真实需求验收。在完成该步骤前，
Harness 的配置默认值继续关闭，也不开放 test/production 部署。
...
ANALYSIS 兼容命令中可调用，而不挂载 Server 时模型侧不可见该 Tool。这些离线证据使用本地确定性
Provider，只关闭 CLI/MCP/取消兼容项；在线 Prompt/Skill 合同另由 `verify-online-provider.sh` 的四阶段
真实调用关闭，但仍不冒充真实需求验收。
...
尚待验证的问题不是建模缺口，而是外部验收条件：真实试点需求和经管理员批准的 local 部署模板。
...
- Runtime 只继承环境 allowlist；`local-login` 额外保留服务账户的 `HOME/CODEX_HOME/XDG_CONFIG_HOME`，让 Codex CLI 自行使用登录态；`isolated-key` 才解析显式 Provider 凭据，只注入单次隔离进程并加入输出脱敏。Secret 值不进入 Snapshot、Prompt、API、SQLite、日志或 Artifact。
...
5. 当前受控本机试点使用正式 Runtime 默认的 `local-login`；生产多用户部署应评估并优先切换为 `isolated-key`，再提供受控凭据引用。
...
| `AGENT_HARNESS_RUNTIME_AUTH_MODE` | `local-login` | 正式 Runtime 模式；可选 `local-login`、`isolated-key` |
| `AGENT_HARNESS_ONLINE_AUTH_MODE` | `local-login` | 仅用于在线验收脚本；可选 `local-login`、`isolated-key` |
| `AGENT_HARNESS_CODEX_CREDENTIAL_REFERENCE` | 空 | 仅 `isolated-key` 使用的环境变量逻辑名；缺失或非法时启动前失败关闭 |
| `AGENT_HARNESS_RUNTIME_TEMP_ROOT` | `data/harness/runtime` | 单次输出/配置临时目录；`isolated-key` 同时把它作为隔离 Home |
...
例如由部署系统提供名为 `HARNESS_CODEX_PROVIDER_KEY` 的 Secret 环境变量时，把
`AGENT_HARNESS_RUNTIME_AUTH_MODE` 设为 `isolated-key`，并把
...
在线 Provider 四阶段 Prompt/Skill 的 `live` 门禁默认直接使用当前用户的 Codex CLI 登录态和 Provider/
模型配置，同时保留 `--ignore-rules`、`--ephemeral`、只读 Sandbox 和临时测试工作区。由于本地 Provider
不一定支持 Codex Output Schema，本模式由 Prompt 要求纯 JSON，再由脚本严格校验字段、Stage 和 Skill
证据：
...
需要验证与正式 Runtime 相同的隔离凭据注入路径时，显式选择 `isolated-key`：
...
报告。两种模式都只验证四阶段真实 Prompt/Skill 到在线 Provider 的只读合同，不替代后续真实需求、
Gate、Approval 和 local 部署验收。模式不合法，或者 `isolated-key` 缺少引用、引用不存在、引用名非法
时，脚本会在启动 Codex 前失败关闭；`isolated-key` 继续使用 Provider Output Schema，两种模式之间不会
静默回退。
