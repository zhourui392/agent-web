...
正式 Harness Runtime 和隔离合同测试没有读取、复制或修改用户级 Codex/Claude 认证目录，也没有在报告中
保存 Key、Token 或其摘要。默认 `local-login` 在线验收由 Codex CLI 自身按正常规则使用当前用户的认证和
Provider/模型配置；脚本不直接读取、复制或改写这些文件。显式 `isolated-key` 模式继续从进程环境解析
逻辑引用，只向单次子进程注入 `OPENAI_API_KEY` 并加入脱敏。
...
当前未完成：一个真实需求的 local 纵向交付，以及正式启用 Runtime 前的受控凭据配置。MVP DoD 与 M4
Exit 因此仍未全部满足，不能仅凭本报告标记 M4 Completed。
