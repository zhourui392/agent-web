...
> 真实 Provider：PASS（`local-login` 四阶段 Prompt/Skill）；真实需求纵向交付尚未完成；未保存凭据或摘要
...
目标机系统默认 Codex CLI 已完成真实 Provider 的四阶段 Prompt/Skill 验收。正式 Harness Runtime 现默认
`local-login`，让系统 `codex` 按服务账户的 `HOME/CODEX_HOME` 使用现有登录态；可显式切换的
`isolated-key` 继续保留临时 Home、Provider Credential Reference、单进程注入与脱敏。两种路径的命令、
环境和结构化输出合同均有聚焦测试。本报告仍保持 M4 未关闭，因为一个真实需求的四阶段、经批准 local
部署、业务 AC 和最终报告尚无通过证据；受控 Stub、在线 Smoke 和 Playwright 不能替代该退出项。
...
19. **在线 Provider live 门禁**：默认 `local-login` 测试先因旧脚本强制要求凭据引用而失败；实现后使用当前用户 Codex 登录态，显式 `isolated-key` 继续使用隔离 Home、Provider Key 和 Output Schema。非法模式，以及隔离模式的无引用、缺失值和非法引用名，均在 Codex 启动前以退出码 2 失败关闭。真实 `codex-cli 0.145.0` 已在默认模式下收到四阶段 Prompt/Skill 并全部通过。
20. **正式 Runtime 本地登录模式**：Infra 测试先因 `AuthMode` 不存在而编译失败；实现后证明默认 `local-login` 不传 `--ignore-user-config`/`--output-schema`、保留服务账户的 Codex 配置目录环境且不解析 Provider 引用；显式 `isolated-key` 保留旧隔离合同并在缺少引用时启动前失败。无 Provider Output Schema 的本地结果由 Java 严格拒绝根对象或 Artifact 的额外字段。
...
本次正式 Runtime `local-login` 修订后的结果：PASS，219 个测试类，1356 tests，0 failures，0 errors，
0 skipped。默认组排除
...
正式 Harness Adapter、验收脚本和测试都不直接读取、复制或修改用户级 Codex/Claude 认证目录，也没有在
报告中保存 Key、Token 或其摘要。默认 `local-login` 由 Codex CLI 自身按正常规则使用服务账户的认证和
Provider/模型配置；显式 `isolated-key` 模式继续从进程环境解析逻辑引用，只向单次子进程注入
`OPENAI_API_KEY` 并加入脱敏。
...
上述 CLI/MCP 合同使用隔离 Home 和本地确定性 Responses Provider。在线 Prompt/Skill 验收另执行：
...
```text
PASS - online Provider received ANALYSIS prompt and domain-modeling-audit Skill
PASS - online Provider received DESIGN prompt and java-ddd-design Skill
PASS - online Provider received IMPLEMENTATION prompt and java-tdd Skill
PASS - online Provider received DEPLOYMENT prompt and release-verification Skill
M4 ONLINE PROVIDER PASS - stages=4 auth_mode=local-login isolated_codex_home=false
```

本机 Provider 在忽略用户配置时认证失败，完整系统默认 Codex 调用成功；同时该 Provider 不支持 Codex
Output Schema。因此 `local-login` 由 Prompt 约束纯 JSON，再由脚本严格执行 `JSON.parse`、字段、Stage、
Skill 证据和 `turn.completed` 校验；`isolated-key` 仍保留 Provider Output Schema。聚焦 Vitest 共 7 个
场景通过，证明两种模式不静默回退。

- [x] 在线 Provider Prompt；
- [x] 在线 Provider 下四阶段真实 Skill 注入；
- [ ] 一个真实需求的四阶段、独立 local Approval、部署、业务 AC 和最终报告。

在线成功路径已经固化为 `scripts/harness-m4/verify-online-provider.sh`。默认模式使用系统 Codex 登录态；
需要复核生产隔离认证路径时，显式设置 `AGENT_HARNESS_ONLINE_AUTH_MODE=isolated-key`，再提供
`AGENT_HARNESS_CODEX_CREDENTIAL_REFERENCE` 指向的环境变量值。
...
当前未完成：一个真实需求的 local 纵向交付。MVP DoD 与 M4 Exit 因此仍未全部满足，不能仅凭本报告标记
M4 Completed；生产多用户试点前还需单独复核 `isolated-key` 的受控凭据路径。
