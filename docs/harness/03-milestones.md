...
| M1 | 建立 Run/Stage/Artifact/Approval 领域内核 | API 可创建和查询 Run，状态可持久化恢复 | 有控制平面，无真实 Agent 执行 |
| M2 | 完成阶段 Prompt 和 Skill 动态装配 | 可预览 Capability Snapshot 和 Prompt Hash | Prompt/Skill 能力平面可用 |
| M3 | 完成只读 MCP 和 Codex Runtime Adapter | 单阶段 Agent 可受控使用指定 Skill/MCP | 执行平面可用 |
...
- [ ] 所有合法/非法状态转换有 Domain 测试。
- [ ] Application 只编排 Repository/Gateway，不承载业务状态判断。
- [ ] SQLite 重启恢复和幂等迁移测试通过。
- [ ] ADMIN 鉴权、幂等键和错误映射测试通过。
- [ ] Feature Flag 关闭时现有接口行为不变。
- [ ] 没有复用或污染现有 Workflow 状态语义。
...
- [ ] 四阶段 Prompt Pack 通过 Schema 和快照测试。
- [ ] 至少两个不同阶段自动选中不同 Skill。
- [ ] 未批准工作区 Skill 不可被启用。
- [ ] Skill 版本冲突、循环依赖和路径逃逸会 fail-closed。
- [ ] 同一输入与资源生成稳定 Snapshot Hash。
- [ ] 资源变化不会改变已运行 Attempt。
...
完成能力平面与执行平面的最小闭环：一个 Stage 可以使用指定 Prompt、Skill 和只读 MCP，由 Codex CLI Runtime 受控执行。
...
- Runtime 兼容性与可强制权限检查；
- 统一 Agent Execution Spec；
- Codex `AgentRuntimeGateway` Infrastructure 适配器；
- 隔离临时配置生成与清理；
- Runtime Execution 持久化、事件归一化、超时和取消；
- Fake MCP、CLI Stub 和真实 CLI 手工验证脚本/说明。
...
- Runtime 启动前必须保存 Snapshot 和 Execution 记录；
- 取消/重试不能重复执行未确认副作用。

### 6.4 TDD 顺序

1. MCP 阶段授权和 Runtime 可执行性策略测试；
2. Application 先保存 Snapshot/Execution 再调用 Gateway 的顺序测试；
3. CLI Stub 的配置转换、事件、超时、取消测试；
4. Fake MCP 的允许/禁止阶段和 Secret Redaction 测试；
5. 真实 CLI 进入显式 `live` 或手工验证，不进入默认快速集。

### 6.5 演示

- ANALYSIS Stage 使用只读 MCP 获取一个需求或文档条目；
- IMPLEMENTATION Stage 如果合同未授权该 MCP，则 Runtime 配置中不存在此 Server；
- 取消正在运行的 Stage 后，Run 进入可恢复状态并保留执行证据；
- API 展示 MCP ID、版本、能力和 Hash，但不展示凭据。

### 6.6 退出门禁

- [ ] Codex Runtime 的 Prompt、Skill、MCP、取消均真实验证。
- [ ] 禁止阶段没有 MCP 配置泄漏。
- [ ] Secret 扫描和 Redaction 测试通过。
- [ ] Runtime 不支持的权限组合启动前失败。
- [ ] 默认测试全部使用 Stub/Fake，不访问真实外部 CLI/MCP。
- [ ] 临时配置在成功、失败、超时和取消后均清理。
...
1. **M0 开始前**：确认 Codex 版本、Fake/只读 MCP 和本机验证边界，不要求提供真实业务需求。
2. **M0 结束**：接受 Codex/MCP 实际约束和 MVP 缩放结果。
3. **M1 结束**：确认 Harness 与现有 Workflow 独立建模。
4. **M3 结束**：确认只读 MCP 的安全边界可以进入纵向试点。
...
文档批准后，先执行 M0，不直接进入 Java 编码：

1. 确认当前 Codex CLI 版本和可丢弃的隔离配置目录；
2. 准备无敏感数据的 Fake/只读 MCP；
3. 准备覆盖四阶段合同的受控样例和 Fixture，不绑定真实业务需求；
4. 验证 Codex 单次运行的 Prompt、Skill、MCP、取消和隔离能力；
5. 用 Spike 结果修订三份文档和 ADR；
6. 再按 TDD 开始 M1 Harness 领域内核；
7. M4 功能集成完成后，再选择真实需求做本机端到端验收。
