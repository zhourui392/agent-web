...
| M4（进行中） | 四阶段能力、真实 Codex 离线合同和受控样例已通过，待在线 Provider/真实需求纵向验收 | 一个真实需求完成本机部署验证 | MVP 发布候选 |
...
- [ ] MVP DoD 全部完成，没有以“后续补”绕过的必需项。（在线 Provider/真实需求项待完成）
- [ ] 一个真实需求生成完整追踪矩阵和最终报告。
- [x] 四阶段中任一上游 Artifact 修改都会使下游结果正确失效。
- [x] 部署失败时安全停止，不会自动重复部署。
- [x] 服务重启后未完成 Run 可恢复，外部动作需要人工对账。
- [x] 最小页面无需改数据库即可完成全流程。
- [x] Feature Flag 关闭回归通过。
- [x] 已记录首版限制和下一阶段优先问题。
...
1. 由用户提供有效的受控 Provider 凭据，并把 `AGENT_HARNESS_CODEX_CREDENTIAL_REFERENCE` 设置为承载该凭据的环境变量名；Harness 不读取或改写用户级认证目录；
2. 执行 `scripts/harness-m4/verify-online-provider.sh` 验证在线 Prompt/四阶段 Skill；真实 Codex 只读 MCP 与取消离线合同已通过；
