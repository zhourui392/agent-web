...
- Harness 可以复用现有 CLI 的进程树、Watchdog、输出上限和原始 JSONL 解码基础，但不能直接复用带 `sessionId/resumeId` 的聊天 `AgentGateway` 或按退出码返回文本的 `AgentCliInvoker`。
- Harness 新建 `app/harness/port/AgentRuntimeGateway`；具体 Codex Adapter 留在 Infrastructure，Application 不新增对 `infra` 具体类的直接依赖。
...
flowchart TB
    ...
    RESOLVER --> SNAPSHOT[Capability Snapshot]
    APP --> ASSEMBLY[Harness Prompt Assembly]
    ASSEMBLY --> SNAPSHOT
    APP --> RUNTIME[Application Agent Runtime Port]
...
| `StageExecution` | 聚合内实体 | 管理阶段状态和 Attempt 历史 |
| `StageAttempt` | 聚合内实体 | 管理固定输入/能力基线的一次尝试，并绑定 Snapshot/Execution 引用 |
| `StageContract` | 值对象 | 定义阶段必需输入、产物、能力请求和门禁 |
| `CapabilitySnapshot` | 不可变写侧模型 | 保存实际授权后的 Prompt/Skill/MCP/Runtime Enforcement 及 Hash |
| `RuntimeExecution` | 聚合根 | 管理外部执行的准备、启动、事件、取消、超时和终态 |
| `ExecutionPermit` | 值对象 | 证明当前 Attempt 和 Snapshot 已通过领域执行前置检查 |
| `McpServerDefinition` | 领域对象 | 表达管理员注册的 Server、能力、风险、阶段和 Runtime 兼容性 |
...
- Snapshot 未固化、已失效或不属于当前 Attempt 时不能启动 Runtime；
- Runtime 成功不等于 Stage 通过，仍必须经过 Artifact、Gate 和 Approval；
- 活动 Runtime 的取消必须先固化取消意图，再终止进程并确认终态；
- 已启动但终态不明的 Execution 不得自动重放；
...
| Application | 创建 Run、开始阶段、解析能力、提交后调用 Runtime、收集证据、事务编排；外部能力端口位于对应 `app/*/port` | 根据状态字符串自行决定转换；遍历 Artifact 重组门禁规则；依赖 Infrastructure 实现 |
| Domain | 生命周期、阶段转换、MCP/Runtime Policy、Execution 状态机、Gate/Approval 策略、失效传播 | 依赖 Spring、SQLite、CLI、MCP SDK、文件系统 |
| Infrastructure | SQLite Repository、Artifact 文件存储、Prompt/Skill/MCP 文件 Catalog、Secret 解析、CLI/MCP/部署适配 | 承载“什么状态允许执行/部署”等业务规则；向内层返回 Secret 明文 |
...
- `RuntimeExecutionRepository`：RuntimeExecution 聚合生命周期，接口位于 Domain。
- `CapabilitySnapshotRepository`：不可变 Snapshot 生命周期，接口位于 Domain。
...
- `PromptPackCatalog`、`SkillCatalog`、`McpServerCatalog`：Domain Catalog 端口，发现已注册能力。
- `AgentRuntimeGateway`：位于 `app/harness/port`，启动、对账、取消 Agent 执行；不暴露具体 CLI 参数。
- `RuntimeExecutionQueryService`：位于 Application，返回执行读模型，Infrastructure 直接投影。
- `CommandExecutionGateway`：执行已经过策略批准的确定性命令，并返回结构化证据。
- `DeploymentGateway`：部署、状态查询和回滚；生产与测试环境可由不同适配器实现。
- Secret Reference 解析与 Redaction：作为 Infrastructure Runtime Adapter 内部能力，不向 Domain/Application 返回明文。
...
stateDiagram-v2
    ...
    CANCELLING --> CANCELLED: runtime termination confirmed
```
...
- RuntimeExecution 使用独立的 `PREPARED/STARTING/RUNNING/CANCEL_REQUESTED/SUCCEEDED/FAILED/TIMED_OUT/CANCELLED/LOST` 状态机；技术执行成功不直接迁移 Stage 为 `PASSED`。
...
id: issue-tracker
version: 1.0.0
description: Read-only issue tracker MCP
stages: [ANALYSIS, DESIGN, IMPLEMENTATION]
runtimes: [CODEX]
command: [issue-tracker-mcp, --stdio]
...
capabilities:
  ...
  - id: issue.update
    ...
    access: WRITE
secrets:
  - environmentVariable: ISSUE_TRACKER_TOKEN
    reference: ISSUE_TRACKER_TOKEN
```

目标能力：

- 注册、启停、健康检查、版本和兼容性校验；
- 按阶段选择 Server 和 Tool，而不是把全部 Server 注入全部 Run；当前 Runtime 不能独立限制 Resource 时，MVP 对含 Resource 的 Server fail-closed；
- 对 READ、WRITE、DEPLOY、SECRET 等风险分类；
- 在 Runtime 支持 Tool allow/deny 时同时下发精确权限；
- Runtime 无法执行精确限制时，对敏感 Server 采取整服不挂载的 fail-closed 策略；
- 使用隔离的临时 Runtime 配置，结束后清理；
- 保存 required、精确 allow/deny、启动/调用超时、配置 Hash、Server 版本和暴露能力清单，不保存 Secret；
- MCP 失败按合同决定阻断或降级，需求源、部署等必需能力不得静默降级。

MCP 使用专门领域模型表达 Server、Tool/Resource、阶段、Runtime、风险、required 和超时，不把这些维度压进 `CapabilityKind.MCP + resource String` 的条件分支。具体模型和求交流程见 [M3 详细设计](04-m3-detailed-design.md#34-mcp-领域模型)。
...
```json
{
  ...
  "workspaceRuntimeInventory": {
    "boundaryKind": "GIT_ROOT",
    "projectConfigAbsent": true,
    "repoSkills": [{
      "id": "java-ddd-design",
      "relativeEntryPath": ".agents/skills/java-ddd-design/SKILL.md",
      "entryHash": "sha256:..."
    }],
  ...
  "mcpServers": [{
    ...
    "version": "1.0.0",
    "required": true,
    "enabledTools": ["issue.read"],
    "disabledTools": ["issue.update"],
    "startupTimeoutSeconds": 10,
    "toolTimeoutSeconds": 30
  }],
  ...
  "runtimeEnforcement": {
    "profileVersion": "codex-runtime-enforcement@1",
    "adapterVersion": "codex-harness-adapter@1",
    "runtimeVersion": "codex-cli 0.145.0",
    "compatibilityMatrixVersion": "m0-2026-07-22",
    "sandboxMode": "read-only",
    "singleRunOverridesEnforced": true,
    "toolAllowDenyEnforced": true,
    "userConfigIsolated": true,
    "projectConfigAbsent": true,
    "repoSkillIsolationEnforced": true,
    "processTreeCancellationEnforced": true
  },
  "policyVersion": "harness-policy-2",
  "snapshotHash": "sha256:..."
}
```

这只是表达形态，不是最终 API Schema。Snapshot 中必须使用规范化后的稳定序列化计算 Hash。已选 Harness Skill 继续保存完整 Package Hash；Codex 自动发现的 Repo Skill 保存 `id + relativeEntryPath + SKILL.md entryHash`，其运行时禁用路径由清单确定性派生。Prompt、Skill/Repo Skill 清单、MCP required/allow/deny/双 timeout 和实际 Runtime Enforcement 必须一次性进入同一个 Snapshot；Runtime 启动时不得旁路追加能力。旧 M2 和缺少安全字段的临时 M3 Snapshot 按原 Schema 只读，原 Hash 不重算；需要 Runtime 执行时必须新建 Attempt。

## 10. Runtime Execution、Adapter 与执行规格

`RuntimeExecution` 是独立聚合，负责一次外部执行的准备、启动、事件、超时、取消和终态；`HarnessRun` 只通过 Execution Reference 关联它。MVP 一个 Attempt 最多绑定一个 RuntimeExecution。
...
- Agent 类型、工作目录和环境；
- 最终 Prompt 与 Hash；
- Skill/MCP Capability Snapshot，包括 Repo Skill 指纹、MCP required、Tool allow/deny 和双 timeout；
- 文件和命令权限；
- 超时、空闲超时、最大输出和取消策略；
- 需要收集的输出契约和证据类型。
...
- 对 Codex 使用只来自 Snapshot 的单次 `-c` 覆盖，不依赖 `--ignore-user-config` 会忽略的用户配置文件；
- 有界探测实际 CLI 版本，阻断未验证版本、祖先 `.codex/config.toml` 和 Snapshot 外 Repo Skill，并在启动前复核工作区指纹；
- 启动、观察、取消并归一化执行事件；
- 返回稳定 Runtime Execution ID 和不透明 PID/进程组 Handle；
...
```text
app/harness/port/AgentRuntimeGateway
    <- infra/harness/CodexHarnessRuntimeGateway
```

现有聊天 `AgentGateway` 和同步 `AgentCliInvoker` 不作为 Harness Runtime 端口；只允许在 Infrastructure 内复用已拆分的进程树、Watchdog、输出上限和原始 JSONL 解码组件。
...
→ 提交事务
→ 调用 AgentRuntimeGateway.start(spec)
```

不能只用 Mockito 调用顺序代替“事务已经提交”的证明。取消同理：先提交 `CANCEL_REQUESTED`，再调用 Gateway 停止进程，终止回调或对账后才确认 `CANCELLED`。
...
Codex 0.145.0 的首版参数与隔离行为已在 M0 验证。M3 重新验收已经按 M0 合同改为单次 `-c`，有界探测实际 CLI 版本，阻断祖先项目配置并禁用已校验的 Repo Skill；不支持版本会 fail-closed。M4 真实试点前仍要执行显式 `live`/手工兼容性验证。Claude 的等价适配在后续里程碑验证。完整执行、故障窗口和取消设计见 [M3 详细设计](04-m3-detailed-design.md)。
...
- Execution 启动幂等按 `(runId, operation, key)` 唯一；相同 key 与相同请求指纹返回原资源及真实状态，不同 Stage 等请求语义复用同一 key 必须冲突。
- 幂等响应的 Location 使用原 Execution 的 Stage/Attempt 构造，不能使用重试请求路径猜测；终态命中不得伪造 `STARTING`。
...
- 有活动 Runtime 时，Run/Stage/Attempt 先进入 `CANCELLING`，RuntimeExecution 进入 `CANCEL_REQUESTED`；确认进程终止后才进入 `CANCELLED`。
...
- Codex MVP 对工作目录到仓库根/批准根祖先链的 `.codex/config.toml` 默认阻断；M3 自动发现 Repo Skill 必须先以 `id + SKILL.md Entry Hash` 匹配可信 Catalog，并通过本次 `skills.config` 全部禁用自动加载；未来允许 CLI 原生加载时再升级为完整 Package Hash 校验。
...
Harness 聚合更新必须增量持久化：Stage/Attempt 已有行按主键更新，新历史追加插入，禁止通过删除并重插 Attempt 保存聚合。Snapshot、RuntimeExecution 和 RuntimeEvent 可以外键引用 Attempt，但只有删除整个 Run 时才允许级联清理。退出测试必须在同一 Attempt 同时绑定 Snapshot 与 RuntimeExecution 后执行 Artifact/Gate/Approval/取消更新，并直接断言两行、Snapshot Hash、`attempt.snapshot_hash` 与 `attempt.execution_id` 均保持不变。

Capability Snapshot 增加 Schema Version、MCP 和 Runtime Enforcement 字段时必须提供幂等老库迁移；`CREATE TABLE IF NOT EXISTS` 不能替代已有表的列迁移。
...
- MCP 授权与 Runtime Enforcement 求交；
- MCP required、Tool allow/deny、双 timeout、不可隔离 Resource 拒绝及其稳定 Snapshot Hash；
- RuntimeExecution 合法/非法状态转换和事件幂等；
- Gate 组合规则；
- 终态、取消中间态、重试和回滚不变量。
...
- 四阶段编排顺序；
- Snapshot 和 PREPARED Execution 在同一事务保存；
- 外部 Runtime 在事务提交后启动；
- Runtime 成功/失败/超时/取消；
- Execution 幂等命中真实状态、跨 Stage key 冲突和 canonical Location；
...
- 真实 SQLite 验证 Repository、幂等迁移，以及 Run 更新后 Snapshot/Execution 不被级联删除；
- `@TempDir` 验证 Prompt/Skill Catalog、Artifact Store 和路径逃逸防护；
- CLI Stub 直接验证 Codex/Claude Runtime Adapter 的单次命令参数、实际版本/PID、项目配置与 Repo Skill 旁路防护；
