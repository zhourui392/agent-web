...
2. 阶段开始前形成不可变 Capability Snapshot。
...
PASSED / FAILED / INVALIDATED / CANCELLED
```
...
首版只把已选 Skill 以 CLI 原生方式或明确指令提供给 Runtime；是否读取 Skill 引用资源由具体 Runtime Adapter 处理。Skill 包计算完整 Hash，至少包含 Manifest、`SKILL.md` 及 Manifest 声明的资源。
...
```text
发现 → 校验信任 → 选择 → 解析最小依赖 → 权限求交 →
Runtime 可执行性检查 → 固化 Snapshot → 开始 Agent 执行
```
...
- MCP ID/Version/能力列表/配置 Hash；
- 文件读写根；
- 逻辑命令白名单；
- 环境、Runtime 和 Policy 版本；
- Snapshot Hash。

一旦阶段开始，不允许原地修改 Snapshot。能力变化必须创建新 Attempt。
...
首版定义统一的 `AgentRuntimeGateway` 概念端口，由 Codex CLI 完成端到端纵向切片。Claude CLI 保持现有业务可用，并通过契约边界保证未来适配不会污染 Domain。
...
```text
POST /api/admin/harness/runs
GET  /api/admin/harness/runs
GET  /api/admin/harness/runs/{runId}
POST /api/admin/harness/runs/{runId}/stages/{stage}/start
POST /api/admin/harness/runs/{runId}/questions/{questionId}/answer
POST /api/admin/harness/runs/{runId}/approvals
POST /api/admin/harness/runs/{runId}/retry
POST /api/admin/harness/runs/{runId}/cancel
GET  /api/admin/harness/runs/{runId}/events
GET  /api/admin/harness/runs/{runId}/artifacts/{artifactId}
GET  /api/admin/harness/runs/{runId}/report
```
...
- 创建 Run 和阶段编排；
- 能力解析后先持久化 Snapshot，再调用 Runtime；
...
- CLI Stub 的 Prompt/Skill/MCP 配置转换；
