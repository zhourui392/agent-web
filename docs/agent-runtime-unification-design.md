...
- [`AgentRuntime`](../src/main/java/com/example/agentweb/app/agentrun/port/AgentRuntime.java) 已定义 `supportedTypes`、`run`、`stop`、Resume ID 提取和 Chunk 归一化。
...
| `RoutingAgentGateway` | 同时承载公共异步生命周期和旧 `AgentGateway` 兼容入口；按 `AgentType` 找到 `AgentRuntime`，维护活动运行和 Stop 竞态语义 | ChatSession、Workbench 聚合、Profile 选择 |
| `AgentRuntime` | Provider/CLI/NATIVE 协议、命令参数、Chunk/Resume 解码、Provider 停止语义 | 业务规则、仓储、用户授权 |
...
| 现有 `AgentRunInvocation`（仅旧兼容路径） | 保持现有 Run/Prompt/History 字段；继续承载 `conversationId`、`userMessageId`、`resumeId`、`env`、`userId`、`history`、`extraEnv` | 只供旧 `AgentGateway`/`AgentRuntime.run` 使用；新公共路径不再把完整 Plan 压缩成该对象 |
...
| 端口 | 输入/输出 | 适用职责 |
| --- | --- | --- |
| `AgentRuntime` | 新路径直接接收 `AgentExecutionPlan` + `RuntimeEventSink`；保留旧 `AgentRunInvocation` 回调方法作为迁移兼容 | Provider 适配、协议解码、历史/Resume、Provider 句柄 |
| `AgentExecutionGateway` | `AgentExecutionPlan` + `RuntimeHandle`/`RuntimeObservation` | 应用统一的异步 Run 生命周期 |

新公共路径不再做 `AgentExecutionPlan → AgentRunInvocation` 的信息压缩。Phase 0 扩展现有 `AgentRuntime`，增加三个 Provider 执行方法：`start(AgentExecutionPlan, RuntimeEventSink)`、`requestStop(RuntimeHandle)`、`observe(RuntimeHandle)`；旧的 `run(AgentRunInvocation, ...)`、`stop(String)`、Chunk/Resume 方法只保留给 `AgentGateway` 迁移兼容。这样 Workspace、Capability、Limits、Attachment、Selection 和 resumeId 原样到达 Provider，字段不会在私有转换器中丢失。

最终由升级后的 `RoutingAgentGateway` 实现 `AgentExecutionGateway`：它只按 `plan.getRuntimeSelection().getAgentType()` 路由到一个 `AgentRuntime`，转发 `start/stop/observe`，并维护旧 `AgentGateway` 的兼容入口。它不读取 Profile、不 materialize Workspace/Capability，也不把 Plan 重组为旧 Invocation。`AgentRuntime` 的新 `start` 方法返回并拥有 Provider 句柄，`RoutingAgentGateway` 只维护 `executionId → AgentRuntime` 的路由表和已完成 Run 的最小终态缓存。

这样保留 `AgentExecutionGateway` 是迁移期的应用端口，而不是再造 Provider Adapter：应用服务暂时不需要改动注入点；所有新的 Provider 执行都已经落到现有 `AgentRuntime` 接口的扩展方法。等旧 `AgentGateway` 消费者全部迁移后，`AgentExecutionGateway` 是否合并进 `AgentRuntime` 再单独评估。

`AgentProcessKernel` 的目标职责是 CLI 进程生命周期；其现有的 Workspace/Capability materialization 仍由 Kernel 在 `AgentExecutionPlan` 上执行，不移动到 `RoutingAgentGateway`。Kernel 接收 `CliAgentRuntime` 已选择的 CLI 方言，负责命令启动、stdin、stdout 监控、watchdog、进程树停止、清理和底层进程观察；它不负责 AgentType 路由、Profile 选择或 Run 级 Stop/Observe。完成覆盖后，旧 `AgentGateway` 调用方逐步迁移到同一个 `RoutingAgentGateway`，`AgentCliGateway` 退役；不新建 `GenericCliProcessRunner`。
...
| 组件 | 改造 | 成本/主要风险 |
| --- | --- | --- |
| `CliAgentRuntime` | 注入已加载的 Profile 配置索引；按 `profileId` 读取 API Key，增加 `start(plan, sink)`，选择方言并委托 Kernel；旧 `run` 暂保留 | 中；自身不重写进程循环，单测需模拟 Profile 索引并验证 Key 不进入 Invocation |
...
- 扩展现有 `AgentRuntime` 的 `start(plan, sink)`、`requestStop(handle)`、`observe(handle)`；保留旧 Invocation 方法作为明确标记的迁移兼容。
...
9. 旧 `AgentCliGateway` 兼容调用和新 `CliAgentRuntime.start(plan, sink)` 共用 `AgentProcessKernel` 与 `CliDialect`，不存在第二个 `ProcessBuilder`/watchdog/process map。
...
| chat-enabled 默认翻转时机不明 | Phase 0 保持 false；Phase 1 先显式环境变量启用，验收后才把 application.yml 默认值改为 true |
...
| CliAgentRuntime 改造成本不明 | 新 `start(plan, sink)` 选择方言并调用 Kernel；旧 `run(invocation)` 仅兼容，AgentCliGateway Phase 1 先改 façade、Phase 3 删除 |
